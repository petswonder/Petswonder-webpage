    ...
    console.log({name})
     ...
     const categoryBox = (n, photo, categoryId, color, url) =>{
        return <div class="col-6 col-sm-2 categorylist">
            ...
            <h6><Link to ={`/pet/${pet}/category/${name}/${url}`} class="title" >{n}</Link></h6>
        </div>
    }

    const FoodCategories=()=>{
        return <div className="container recent-game-section spad set-bg"><div className="row">
                ...
                {categoryBox("Snacks", food, 4, "#fca3cc", "Snacks")}
        </div>
        </div>
    }
